...
void list_add_ascending_by_address(list_t *l, block_t *newblk){
  ...
  if(l->head == NULL){
    ...
    //Case if there is a single node in the list.
    if(current_node -> next == NULL){
      if(newblk -> start <= current_node -> blk -> start){
        new_node ->next = l -> head;
        l ->head = new_node;
      }
      
      //Otherwise newblk goes after the current node
      else{
    ...
    //If there are multiple nodes in list
      ...
      if(new_node ->blk -> start <= current_node -> blk -> start){
        new_node ->next = l-> head;
        l->head = new_node;
      }
      
      else {
        while(current_node != NULL && new_node -> blk -> start > current_node -> blk -> start){
...
void list_add_ascending_by_blocksize(list_t *l, block_t *newblk){
  ...
  if(l->head == NULL){
    ...
    previous_node = current_node = l->head;
    
    curblk_size = current_node->blk->end - current_node->blk->start + 1;
    
    if(current_node->next == NULL) {  //only one node in list
       ...
       if(newblk_size >= curblk_size) {  // place in front of current node
          ...
          while(current_node != NULL && newblk_size <= curblk_size) {
               previous_node = current_node;
               current_node = current_node->next;
               
               if(current_node != NULL)  // the last one in the list
                     curblk_size = current_node->blk->end - current_node->blk->start;
          }
          previous_node->next = new_node;
          new_node->next = current_node;
       }
...
void list_coalese_nodes(list_t *l){ 
  ...
  node_t *current_node = l ->head -> next;
  
  while(current_node){
    if(previous_node ->blk ->end +1 == current_node -> blk -> start){
      previous_node ->blk -> end  = current_node -> blk -> end;
      previous_node -> next = current_node -> next;
      
      free(current_node -> blk);
      free(current_node);
