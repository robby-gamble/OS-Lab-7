...
void allocate_memory(list_t * freelist, list_t * alloclist, int pid, int blocksize, int policy) {
  ...
  if(!memory_block){
    ...
    if(!previous){
      ...
      previous->next = memory_block->next;
    }
  }
  ...
  //3. set the blk.pid = pid
  int memory_block_end = memory_block -> blk ->end;
  memory_block -> blk -> pid = pid;
  
  //4. set the blk.end = blk->start + blocksize - 1
  memory_block -> blk -> end = memory_block -> blk -> start +blocksize - 1;
  
  //5. add the blk to the ALLOC_LIST in ascending order by address.
  list_add_ascending_by_address(alloclist, memory_block ->blk);
  
  //6. Deal with the remaining left over memory (fragment).
  if (memory_block -> blk ->end != memory_block_end){
    //a. dynamically allocate a new block_t called fragment [use malloc]
    block_t *fragment = (block_t*)malloc(sizeof(block_t));
    //b. set the fragment->pid = 0 
    fragment ->pid = 0;
    //c. set the fragment->start = the blk.end + 1
    fragment ->start = memory_block -> blk -> end + 1;
    //d. set the fragment->end = original blk.end before you changed it in #4
    fragment -> end = memory_block_end;
    //e. add the fragment to the FREE_LIST based on policy
    if(policy == 1){
...
void deallocate_memory(list_t * alloclist, list_t * freelist, int pid, int policy) { 
  ...
  node_t *previous = NULL;
  block_t *deallocate_memory_blk = NULL;
  
  //1. Check if a node is in the ALLOC_LIST with a blk.pid = pid
  while(current -> blk -> pid != pid){
    previous = current;
    current = current -> next;
  }
  ...
  if(!previous){
    ...
    deallocate_memory_blk = current -> blk;
  }
  else {
    previous -> next = current ->next;
    deallocate_memory_blk = current ->blk;
  }
  deallocate_memory_blk -> pid = 0;
  // 4. add the blk back to the FREE_LIST based on policy.
  if(policy == 1){
      list_add_to_back(freelist, deallocate_memory_blk);
    }
    else if(policy == 2){
      list_add_ascending_by_blocksize(freelist,deallocate_memory_blk);
    } 
    else if(policy == 2){
      list_add_descending_by_blocksize(freelist,deallocate_memory_blk);
    }
